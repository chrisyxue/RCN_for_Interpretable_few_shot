...
parser.add_argument("-width","--width",type=int,default=5)
...
name = "V6_new_res_argue5_meta_pair_N_" + str(CLASS_NUM) + "_S_" + str(SAMPLE_NUM_PER_CLASS) + "_B_" + str(BATCH_NUM_PER_CLASS) + "_l_" + str(LEARNING_RATE) 
...
def main():
    ...
    explain_network = ExplainModule_v6_meta_pair(n_way=CLASS_NUM,k_shot=SAMPLE_NUM_PER_CLASS,width=WIDTH,batch_size=BATCH_NUM_PER_CLASS)
    ...
    feature_encoder_scheduler = ReduceLROnPlateau(feature_encoder_optim,mode="max",factor=0.5,patience=2,verbose=True,threshold=1e-2)
    explain_network_optim = torch.optim.Adam(explain_network.parameters(),lr=LEARNING_RATE)
    explain_network_scheduler  = ReduceLROnPlateau(explain_network_optim,mode="max",factor=0.5,patience=2,verbose=True,threshold=1e-2)
    ...
    for episode in range(EPISODE):
        ...
        distance_feature = middle_layer.forward(support_x=sample_features_ext,query_x=batch_features_ext) # [n*b,n*s,19*19]
        pre = explain_network(distance_feature,torch.cat([sample_features_ext,batch_features_ext],dim=2)) # [n*b,n]
        # print(pre.size())
            ...
            with torch.no_grad():
                for i in range(TEST_EPISODE):
                    ...
                    for val_images,val_labels in val_dataloader:
                        ...
                        distance_feature = middle_layer.forward(support_x=sample_features_ext,query_x=val_features_ext) # [n*b,n*s,19*19]
                        # distance_feature = distance_feature.view([CLASS_NUM*num_per_class,CLASS_NUM*SAMPLE_NUM_PER_CLASS*9*9])
                        pre = explain_network(distance_feature,torch.cat([sample_features_ext,val_features_ext],dim=2)) # [n*b,n]
                        # print(pre[0])
                        # print(test_labels[0])
            ...
            with torch.no_grad():
                for i in range(TEST_EPISODE):
                    ...
                    for test_images,test_labels in test_dataloader:
                        ...
                        distance_feature = middle_layer.forward(support_x=sample_features_ext,query_x=test_features_ext) # [n*b,n*s,19*19]
                        # distance_feature = distance_feature.view([CLASS_NUM*num_per_class,CLASS_NUM*SAMPLE_NUM_PER_CLASS*9*9])
                        pre = explain_network(distance_feature,torch.cat([sample_features_ext,test_features_ext],dim=2)) # [n*b,n]
